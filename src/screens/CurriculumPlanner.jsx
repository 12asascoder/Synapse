...
const STYLES = {
  ...
  moduleTitle: { fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 6px' },
  progressBar: { height: '8px', borderRadius: '4px', background: 'var(--bg-card)', marginTop: '8px', overflow: 'hidden', border: '1px solid var(--border-subtle)' },
  ...
  scoreBar: { height: '6px', borderRadius: '3px', background: 'var(--bg-card)', marginTop: '8px', overflow: 'hidden', border: '1px solid var(--border-subtle)' },
    ...
    <div style={outerStyle}>
      ...
      <div style={rightColStyle}>
        <div style={STYLES.container}>
          ...
          {adjustmentResult && (
            <div style={{ ...STYLES.statCard, marginBottom: 20, background: 'var(--bg-card)', borderColor: 'var(--border-subtle)' }}>
              <p style={{ color: 'var(--border-active)', fontWeight: 600, marginBottom: 8 }}>Difficulty Adjusted</p>
              {adjustmentResult.adjustments?.map((a, i) => {
                const dc = difficultyColors[a.difficulty] || difficultyColors.standard;
                return (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 4, padding: '4px 0' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <h3 style={{ fontSize: 16, color: 'var(--text-primary)', marginBottom: 12 }}>Mastery Breakdown</h3>
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Quiz: {Math.round((masteryBreakdown.quizWeight || 0.4) * 100)}%</span>
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Exercise: {Math.round((masteryBreakdown.exerciseWeight || 0.3) * 100)}%</span>
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Efficiency: {Math.round((masteryBreakdown.efficiencyWeight || 0.15) * 100)}%</span>
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Consistency: {Math.round((masteryBreakdown.consistencyWeight || 0.15) * 100)}%</span>
            </div>
          </div>

          <div>
            <h3 style={{ fontSize: 16, color: 'var(--text-primary)', marginBottom: 12 }}>Modules</h3>
            {modules.map((mod, i) => {
              const modProgress = progress.currentModule > i ? 100 : progress.currentModule === i ? 50 : 0;
              const isComplete = progress.currentModule > i;
              const dc = difficultyColors[mod.difficulty] || { bg: 'var(--bg-card)', color: 'var(--border-active)' };
              ...
              return (
                <div key={mod.id || i} style={{ ...STYLES.moduleCard, opacity: modProgress === 0 && !isComplete ? 0.6 : 1 }}>
                  ...
                  <div style={STYLES.progressBar}>
                    <div style={{ ...STYLES.progressFill, width: `${modProgress}%`, background: modProgress >= 80 ? '#6ee7b7' : 'var(--border-active)' }} />
                  </div>
                  {mod.competencies?.length > 0 && (
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
                      {mod.competencies.map((c, j) => (
                        <span key={j} style={{ ...STYLES.badge, background: 'var(--bg-card)', color: 'var(--border-active)', textTransform: 'none', border: '1px solid var(--border-subtle)' }}>{c}</span>
                      ))}
                    </div>
                  )}
                  ...
                  {hasRemedial && (
                    <div style={{ marginTop: 12, padding: 12, background: 'rgba(245,158,11,0.08)', borderRadius: 8, border: '1px solid rgba(245,158,11,0.2)' }}>
                      ...
                      {mod.remedial.recommendedActions?.map((a, j) => (
                        <p key={j} style={{ fontSize: 12, color: 'var(--text-secondary)', margin: '0 0 4px' }}>&bull; {a}</p>
                      ))}
                      {mod.remedial.resources?.map((r, j) => (
                        <div key={j} style={{ marginTop: 4, padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: 6 }}>
                          <p style={{ fontSize: 12, color: 'var(--text-primary)', margin: 0 }}>{r.topic}: {r.description}</p>
                          <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: 0 }}>{r.estimatedTime}</p>
                        </div>
                      ))}
          ...
          {plan.skillGap?.length > 0 && (
            <div style={{ marginTop: 32 }}>
              <h3 style={{ fontSize: 16, color: 'var(--text-primary)', marginBottom: 12 }}>Skill Gap Analysis</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {plan.skillGap.map((gap, i) => (
                  <div key={i} style={{ ...STYLES.statCard, padding: '12px 16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ color: 'var(--text-primary)', fontSize: 14 }}>{gap.name}</span>
            ...
            <div style={STYLES.modal} onClick={() => setRemedialResult(null)}>
              <div style={{ ...STYLES.statCard, maxWidth: 500, margin: 'auto' }} onClick={e => e.stopPropagation()}>
                ...
                {remedialResult.needsRemedial && (
                  <div>
                    <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 8 }}>Score: {remedialResult.remedial.score}% (threshold: {remedialResult.remedial.threshold}%)</p>
                    {remedialResult.remedial.recommendedActions?.map((a, i) => (
                      <p key={i} style={{ color: 'var(--text-secondary)', fontSize: 13, margin: '0 0 4px' }}>&bull; {a}</p>
                    ))}
                  </div>
                )}
