...
const STYLES = {
  ...
  tabs: { display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid rgba(99,102,241,0.15)', paddingBottom: 0 },
  tab: {
...
function OvePractice({ prep, onAnswer }) {
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#64748b', marginBottom: 16 }}>Opinion · Vision · Experience — deep dive questions</p>
        ...
        <div key={i} style={STYLES.questionCard}>
          ...
          {a.strengths?.length > 0 && (
            <div style={{ marginTop: 8 }}>
              ...
              {a.strengths.map((s, j) => <p key={j} style={{ fontSize: 12, color: '#94a3b8', margin: 0 }}>• {s}</p>)}
            </div>
          )}
          {a.weaknesses?.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <p style={{ fontSize: 12, color: '#fca5a5', fontWeight: 600, marginBottom: 4 }}>Areas to Improve</p>
              {a.weaknesses.map((w, j) => <p key={j} style={{ fontSize: 12, color: '#94a3b8', margin: 0 }}>• {w}</p>)}
            </div>
          )}
        </div>
      ))}
...
function PassportView({ passport }) {
  ...
  return (
    <div style={{ ...STYLES.statCard, marginTop: 24 }}>
      <h3 style={{ color: '#f1f5f9', fontSize: 20, marginBottom: 4 }}>Skill Passport</h3>
...
export default function InterviewPrep() {
  ...
  const handleAnswer = async (prepId, questionId, question, answer, type) => {
    const result = await apiPost('/interview/answer', { prepId, questionId, question, answer, type }, state.token);
    fetchPrep();
    return result;
  };
    ...
    <ThemeContainer>
      <div style={STYLES.container}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            ...
            <p style={STYLES.subtitle}>
              {prep.jdSummary || prep.targetRole || 'Interview Preparation'}
        ...
        <div style={STYLES.tabs}>
          <button style={{ ...STYLES.tab, ...(tab === 'star' ? STYLES.tabActive : {}) }} onClick={() => setTab('star')}>STAR Practice</button>
          <button style={{ ...STYLES.tab, ...(tab === 'ove' ? STYLES.tabActive : {}) }} onClick={() => setTab('ove')}>OVE Deep Dive</button>
          <button style={{ ...STYLES.tab, ...(tab === 'feedback' ? STYLES.tabActive : {}) }} onClick={() => setTab('feedback')}>Feedback</button>
          {prep.status === 'completed' && prep.passport && (
            <button style={{ ...STYLES.tab, ...(tab === 'passport' ? STYLES.tabActive : {}) }} onClick={() => setTab('passport')}>Passport</button>
          )}
        </div>
